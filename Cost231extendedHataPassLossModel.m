...
function [ path_loss ] = Cost231extendedHataPassLossModel( distance_UE_BS, type_environment,ID,timeslot,subcarrier)
global shadowing
...
if type_environment == 'urban'
    ...
    path_loss = 46.3 + 33.9*log10(frequency) - 13.82*log10(h_base) - a + (44.9 - 6.55*log10(h_base))*log10(distance_UE_BS) + Cm + shadowing(ID,timeslot,subcarrier); 
                
elseif type_environment == 'rural'
       h_base= 45;
                
       path_loss = 69.55 + 26.16*log10(frequency) - 13.82*log10(h_base) + ...
           (44.9 - 6.55*log10(h_base))*log10(distance_UE_BS) - 4.78*(log10(frequency))^2 + 18.33*log10(frequency)-40.94 + shadowing(ID,timeslot,subcarrier);                      
end
